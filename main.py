...
books = [
     ...
     Book(
          book_id=9,
          title="Saxon Math Homeschool 7/6",
          author="Hake Saxon",
          year=2004,
     ...
     Book(
          book_id=13,
          title="Saxon Math 1 (Saxon Math Grade 1)",
          author="Nancy Larson",
          year=2004,
          publisher="Saxon, a trademark of Harcourt Achieve",
     ),
     Book(
          book_id=14,
          ...
          year=2007,
          publisher="Yediʻot aḥaronot",
     ),
     Book(
          book_id=15,
          title="Probability and statistics with R",
          author="María Dolores Ugarte",
          year=2008,
...
@api.put("/{book_id}/cover")
def update_cover(book_id: int, cover_image: UploadFile) -> dict:
     for book in books:
          if book.book_id == book_id:
               coverPath = f"images/{book_id}"
